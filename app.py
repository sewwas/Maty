...
def get_global_vps_trading_engine_v4():
    ...
    for sym in _symbols:
        ...
        bot = BreakoutGridBot(
            ...
            order_size_multiplier=pair_cfg.get("lot_mult", 1.0),
            target_profit=pair_cfg.get("min_tp", 6.0),
            is_percent=True,
            auto_restart=True,
        ...
        with col1:
            ...
            with d_col1:
                if st.button("FLATTEN POSITIONS", type="primary", key="man_flat_btn", use_container_width=True):
                    ...
                    # Fully reset cycle tracking so daemon won't auto-redeploy
                    bot._max_open_in_cycle = 0
                    bot.deployed = False
                    bot._manual_tp_just_fired = False
            ...
            with d_col2:
                if st.button("CANCEL ALL TRAPS", key="man_canc_btn", use_container_width=True):
                    ...
                    # Reset cycle counter so daemon won't think it needs to restart
                    bot._max_open_in_cycle = 0
                    bot._manual_tp_just_fired = False
