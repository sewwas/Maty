...
class ManualGridBot(BreakoutGridBot):
    ...
    def process_tick(self, previous_price: float, current_price: float, timestamp: float, bb_width: Optional[float] = None) -> Optional[dict]:
        ...
        # 2. Check Basket PNL Take Profit & Stop Loss (MANUAL ONLY — no auto engine interference)
        if current_open > 0:
            ...
            target_usd = float(getattr(self, "target_profit", 15.0) or 0.0)
            sl_usd = float(getattr(self, "stop_loss", 0.0) or 0.0)

            # Diagnostic: log basket state every 10 ticks so we can trace the TP/SL check
            self._manual_tick_ctr = getattr(self, "_manual_tick_ctr", 0) + 1
            if self._manual_tick_ctr % 10 == 0:
                print(f"[{self.symbol}] 📊 [MANUAL BASKET] Open={current_open} Pending={current_pending} | PnL=${total_pnl:.2f} (raw={total_pnl_raw:.2f}) | Target=${target_usd:.2f} | SL=${sl_usd:.2f} | Cent={is_cent}")

            # 2A. Basket Target Profit Hit
            if target_usd > 0 and total_pnl >= target_usd:
                ...
                # Verify broker truly has 0 open positions before redeploying
                post_close_open = len(getattr(self.broker, "open_positions", {}))
                if post_close_open > 0:
                    print(f"[{self.symbol}] ⏳ [MANUAL TP] {post_close_open} position(s) still open after close — will retry next tick.")
                    return None

                # Reset cycle state and redeploy same grid direction
                self._max_open_in_cycle = 0
                self._manual_tp_just_fired = True   # Guard: suppress restart branch this tick
                self.deployed = False
                if getattr(self, "auto_restart", True):
                    self.deploy_traps(current_price, timestamp, force=True)
                return None

            # 2B. Basket Stop Loss Hit
            if sl_usd > 0 and total_pnl <= -abs(sl_usd):
                print(f"[{self.symbol}] 🛑 [MANUAL SL HIT] Basket loss ${total_pnl:.2f} <= -${sl_usd:.2f}. Closing all positions & cancelling traps.")
                try:
                    self.broker.close_all_positions(symbol=self.symbol)
                    self.broker.cancel_all_orders(symbol=self.symbol)
                except Exception as e:
                    import logging; logging.warning(f"Basket SL close error: {e}")

                if hasattr(self, "record_trade_outcome"):
                    self.record_trade_outcome(total_pnl, "MANUAL_BASKET_SL", 0)
                ...
                self._max_open_in_cycle = 0
                self._manual_tp_just_fired = True
                self.deployed = False
                if getattr(self, "auto_restart", True):
                    self.deploy_traps(current_price, timestamp, force=True)
                return None

        # 3. If positions closed externally (e.g. SL hit / manual close from UI) and
        #    traps remain pending — cancel old traps and redeploy a fresh grid.
        #    Guard: skip this branch for 1 tick right after a TP redeploy to avoid
        #    double-firing on the freshly placed pending orders.
        if getattr(self, "_manual_tp_just_fired", False):
            self._manual_tp_just_fired = False  # Clear the guard after one tick
        elif getattr(self, "_max_open_in_cycle", 0) > 0 and current_open == 0 and current_pending > 0:
            print(f"[{self.symbol}] 🔄 [MANUAL RESTART] All positions closed. Cancelling stale traps & redeploying manual grid.")
            try:
                self.broker.cancel_all_orders(symbol=self.symbol)
            except Exception:
                pass
